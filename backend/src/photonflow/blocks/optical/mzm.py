...
class MZM(BaseBlock):
    ...
    def process(self, inputs: Dict[str, Signal], ctx: SimContext) -> Dict[str, Signal]:
        ...
        if drive_mode == "push_pull":
            phi1 = (phi_bias + bias_error) + 0.5 * math.pi * v / vpi
            phi2 = (-phi_bias + bias_error) - 0.5 * math.pi * v / vpi + phase_err
        else:
            phi1 = (phi_bias + bias_error) + math.pi * v / vpi
            phi2 = (-phi_bias + bias_error) + phase_err

        e1 = torch.exp(1j * phi1)
        e2 = arm_ratio * torch.exp(1j * phi2)
        data = 0.5 * opt_in.data * (e1 + e2)
